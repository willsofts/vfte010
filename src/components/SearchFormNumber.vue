<template>
<div id="searchpanelnumber" class="panel-body search-panel">
    <div class="row row-height">
        <div class="col-height col-md-3">
            <label>{{ labels.reservenums_label }}</label>
            <input type="text" v-model="localData.reservenum" class="form-control input-md" maxlength="50" />
        </div>
        <div class="col-height col-md">
            <br/>
            <button id="searchbuttonnum" @click="searchClick" class="btn btn-dark btn-sm btn-ctrl"><i class="fa fa-search fa-btn-icon" aria-hidden="true"></i>{{ labels.search_button }}</button>
            <button id="resetbuttonnum" @click="resetClick" class="btn btn-dark btn-sm btn-ctrl"><i class="fa fa-refresh fa-btn-icon" aria-hidden="true"></i>{{ labels.reset_button }}</button>
            <button id="insertbuttonnum" @click="insertClick" class="btn btn-dark btn-sm btn-ctrl pull-right"><i class="fa fa-plus fa-btn-icon" aria-hidden="true"></i>{{ labels.insert_button }}</button>
        </div>
    </div>
...
export default {
  ...
  methods: {
    ...
    collecting(options,criterias) {
      ...
      startWaiting();
      $.ajax({
        url: getApiUrl()+APP_URL+"/collectnum",
